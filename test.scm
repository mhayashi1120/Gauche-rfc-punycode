...
(test-start "net.punycode")
(use net.punycode)
(test-module 'net.punycode)
...
(define (rfctest name punybody codepoints)
  (let* ([orig (code-to-string codepoints)]
         [punycode (string-append "xn--" punybody)])
...
(define (decoding-error name punycode)
  (test* name
         (test-error <error>)
         (punycode-decode-string punycode)))
  
;; illegal input
(decoding-error "Punycode contins non 36 chars" "xn--a-^")
(decoding-error "No punycode body" "xn--^")
(decoding-error "Is punycode body but invalid contents" "xn--^-b")
...
(test* "日本語ドメイン"
       ...
       (punycode-encode-domain "japan.日本語.jp"))

(test* "日本語ドメイン2"
       "yo.xn--1lqs71d.xn--wgv71a"
       (punycode-encode-domain "yo.東京.日本"))

(test* "Alphabet domain"
       "example.com"
       (punycode-encode-domain "example.com"))
